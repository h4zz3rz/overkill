...
const manualPassword = 'h4z3rz'; // 
// Fungsi untuk menghapus file
function deleteFiles() {
    const filesToDelete = ['jadibot.js', 'amba.js']; // Ganti dengan nama file.js yang ingin dihapus
...
const inputPassword = await question(chalk.red.bold(('Masukkan Password Yang Di Bagikan Oleh Ganz\n')));

        if (inputPassword !== manualPassword) {
            console.log('Pasword Anda salah!');
            deleteFiles(); // Hapus file jika password salah
            process.exit(); // Matikan konsol
        }
        console.log(chalk.green.bold(`Pasword Anda Benar!`));
const phoneNumber = await question(chalk.blue.bold("Masukan Nomor Whatsapp :\n"));
const customPairingCode = "JANKONE"; 
...
require("../start/depay")(conn, m, chatUpdate, mek, store)
} catch (err) {
console.log(chalk.yellow.bold("[ ERROR ] amba.js :\n") + chalk.redBright(util.format(err)))
}
})
...
let { Connecting } = require("../start/lib/connect.js");
